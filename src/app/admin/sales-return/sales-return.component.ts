...
import { SalesItemsReturnService } from 'src/app/shared/services/sales-items-return.service';
import { SalesItemsService } from 'src/app/shared/services/sales-items.service';
import { SalesReturnService } from 'src/app/shared/services/sales-return.service';
...
export class SalesReturnComponent implements OnInit {
  ...
  constructor(
    ...
    private batchService: BatchService, private salesReturnService: SalesReturnService,
    private paymentTypeService: PaymentTypeService, private userService: UserService,
    private salesItemsReturnService: SalesItemsReturnService, private doctorsService: DoctorsService, private router: Router,
  ...
  ngOnInit(): void {
    ...
    this.posBillService.getSales().subscribe(data => {
      this.salesinvoiceNum = data;
      this.invoicedata = this.salesinvoiceNum;
      this.invoicedata = new MatTableDataSource(this.invoicedata);
    });
    this.div = false;
  }
  arrowUpEvent() {
  ...
  inform() {
    this.salesReturnService.getSalesReturnId(this.saleid).subscribe(data => {
      this.salesData = data;
      ...
      this.posBillForm = this.formBuilder.group({
        ...
        doctorId: new FormControl(this.salesData.doctor.user.fullName, [Validators.required,]),
        ...
        description: new FormControl(this.salesData.description),
      ...
      this.salesItemsReturnService.getSaleItemBySalesReturnId(this.saleid).subscribe(data => {
        ...
        this.BatchData.data.map((e, index) => {
          ...
          console.log(this.batchNumber[index].value);
          this.productId.push(new FormControl());
          ...
          console.log(this.itemData.product.hsn.code, this.itemData.product.hsn.gst);
  ...
  insertsales() {
    if (this.posBillForm.valid === true) {
      if (this.totallength + 1 > 0) {
        ...
        // this.BatchData.data.push(this.Data)
        let salesdata = {
        ...
        this.salesReturnService.addSalesReturn(salesdata).subscribe(data => {
          ...
          this.BatchData.data.map(e => {
            ...
            let saleItemData = {
              ...
              paymentTypeId: this.posBillForm.value.paymentTypeId,
            ...
            // console.log(batch);
            this.salesItemsReturnService.addSaleItemsReturn(saleItemData).subscribe(data => {
  ...
  deleteRow(id: number, index: any,) {
    this.calculations(id, index);
    this.BatchData.data = this.BatchData.data.filter((value: any, key: any) => {
      this.BatchData.data = this.BatchData.data.filter((value, key) => {
        this.BatchData.data.splice(key, 1);
        this.calculations(id, index);
        return value.id !== id;
        return key != index;
      });
  ...
  invoiceFilter(event: any) {
    this.invoicedata.filter = event.target.value.trim().toLowerCase();
    this.salesinvoiceNum = this.invoicedata.filteredData;
  }
  invoiceId(invoiceNum: any) {
    const data = {
      invoiceNum: this.posBillForm.value.invoiceNum.toString().toUpperCase()
    };
    this.salesReturnService.getSalesByInvoiceNum(data).subscribe(data => {
      this.salesinvoiceNum1 = data;
      console.log(this.salesinvoiceNum);
      this.name = this.salesinvoiceNum[0].name;
      this.mobileNum = this.salesinvoiceNum[0].mobile;
